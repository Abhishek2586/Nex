...
imports_status = {name:importlib.util.find_spec(name) is not None for name in modules}
report={
...
raise SystemExit(0 if all(imports_status.values()) and report['frontend_built'] and report['models_prepared'] else 1)
