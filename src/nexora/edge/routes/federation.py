...
def data_status():
    ...
    wesad_eval_path = ROOT / 'evidence/recorded/WESAD_EVALUATION.json'
    ...
    if wesad_eval_path.exists():
        try:
            ...
            # New format: provenance nested under dataset_provenance
            prov = m.get("dataset_provenance", {})
            dataset_hash = prov.get("dataset_sha256", m.get("dataset_sha256", ""))
            subject_count = prov.get("imported_subject_count", m.get("imported_subject_count", 0))
            sp = prov.get("splits", {})
            train = sp.get("train", m.get("train_subjects", []))
            val = sp.get("validation", m.get("validation_subjects", []))
            test = sp.get("test", m.get("test_subjects", []))
