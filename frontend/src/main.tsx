...
function App(){
 ...
 const refresh=async()=>{try{const [h,s,p,m,am,r,v,proj,dstat]=await Promise.all([api('health'),api('sessions'),api('interventions'),api('metrics'),api('models/active').catch(()=>null),api('experiments'),api('privacy'),api('privacy/projection'),api('data/status').catch(()=>({wesad: {status: 'NOT RUN'}}))]);h.wesad = dstat.wesad;H(h);S(s);P(p);M(m);AM(am);R(r);V(v);PROJ(proj);if(h.coordinator==='available'){try{J(await api('experiment-jobs'))}catch{J([])}}else J([]);ERR('');if(s.length)SEL(prev=>prev?prev:s[0].id);}catch(e){ERR(String(e));H({status:'disconnected'});}};
 ...
 {page==='AI insights'&&mode==='research'&&<><div className="card"><h2>Dataset Insights</h2>
  <label style={{marginRight: '10px'}}>Dataset:
    <select value={explainModel.includes('wesad') ? 'wesad' : 'synthetic'} onChange={e=>{
      const isWesad = e.target.value === 'wesad';
      XM(isWesad ? 'neural_wesad' : 'neural');
      X(null);
    }}>
 ...
 <div className="twocol">
  {models.filter(m => (explainModel.includes('wesad') ? m.source === 'Recorded dataset' : m.source === 'Synthetic')).map(m=><div className="card" key={m.model_hash}><span className="badge">{m.source} · {m.lifecycle_state||'candidate'}</span><h2>{m.model_id==='baseline'?'Random forest':'Neural network'}</h2><strong className="metric">{(100*(m.validation_score || m.metrics?.balanced_accuracy || 0)).toFixed(1)}%</strong><p>Validation Balanced accuracy</p><p>Test Balanced Acc: {(100*(m.test_balanced_accuracy || m.metrics?.balanced_accuracy || 0)).toFixed(1)}% · Macro-F1: {(m.test_macro_f1 || m.metrics?.macro_f1 || 0).toFixed(3)}</p><p>Architecture: {m.architecture_id || 'Unavailable'}</p><p>Hash: {m.model_hash}</p><p>Dataset Hash: {m.dataset_hash}</p><div><h4>Confusion Matrix</h4><table style={{width:'100%', textAlign:'center'}}><thead><tr><th>Actual \ Pred</th><th>Baseline</th><th>Stress-like</th></tr></thead><tbody><tr><td>Baseline</td><td>{m.metrics?.confusion_matrix?.[0]?.[0]}</td><td>{m.metrics?.confusion_matrix?.[0]?.[1]}</td></tr><tr><td>Stress-like</td><td>{m.metrics?.confusion_matrix?.[1]?.[0]}</td><td>{m.metrics?.confusion_matrix?.[1]?.[1]}</td></tr></tbody></table></div><div className="alert-box" style={{marginTop:'1rem', padding:'1rem', background:'#ffeeba', color:'#856404'}}><strong>{m.source === 'Recorded dataset' ? 'Recorded WESAD research dataset result — not clinical validation.' : 'Synthetic simulator result only'}</strong></div></div>)}
 </div><div className="card"><h2>Per-prediction attribution</h2><label>Model<select value={explainModel.replace('_wesad', '')} onChange={e=>{XM(explainModel.includes('wesad') ? e.target.value + '_wesad' : e.target.value);X(null)}}><option value="neural">Neural network</option><option value="baseline">Random forest</option></select></label><button disabled={!pred?.prediction_id} onClick={()=>act(async()=>X(await api(`explanations/${pred.prediction_id}?model=${explainModel.replace('_wesad', '')}`)))}>Explain latest prediction</button>{explanation&&<><p>{explanation.method} · {explanation.output_space} · class {explanation.class}</p>
 ...
 {page==='Evidence'&&mode==='research'&&<div className="card"><h2>Evidence package</h2>
  {health.wesad && health.wesad.status === 'COMPLETED' ? (
    <div style={{background:'var(--surface)',borderRadius:'0.5rem',padding:'1rem',marginBottom:'1rem'}}>
      ...
      <p><strong>Measured test metrics (Baseline):</strong> Balanced Acc: {((models.find(m => m.model_id === 'baseline' && m.source === 'Recorded dataset')?.test_balanced_accuracy || 0) * 100).toFixed(1)}%</p>
      <p><strong>Federated run status:</strong> {runs.find(r => r.mode === 'federated' && r.dataset === 'wesad')?.status || 'NOT RUN'}</p>
      <p><strong>Private federated run status:</strong> {runs.find(r => r.mode === 'private-federated' && r.dataset === 'wesad')?.status || 'NOT RUN'}</p>
    </div>
  ) : (
    <div style={{background:'var(--surface)',borderRadius:'0.5rem',padding:'1rem',marginBottom:'1rem'}}>
