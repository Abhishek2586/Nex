...
function App(){
 ...
 const refresh=async()=>{try{const [h,s,p,m,r,v,proj]=await Promise.all([api('health'),api('sessions'),api('interventions'),api('metrics'),api('experiments'),api('privacy'),api('privacy/projection')]);H(h);S(s);P(p);M(m);R(r);V(v);PROJ(proj);if(h.coordinator==='available'){try{J(await api('experiment-jobs'))}catch{J([])}}else J([]);ERR('');if(!selected&&s.length)SEL(s[0].id);}catch(e){ERR(String(e));H({status:'disconnected'});}};
 ...
 const activePrompts=prompts.filter(p=>p.status==='offered'||p.status==='accepted');
 const activeModel = models.find(m => m.lifecycle_state === 'active') || models[0];
 ...
 <div style={{display:'flex', gap:'1rem', overflowX:'auto', padding:'1rem 0'}}>
   ...
   <div className="pipeline-node"><b>Policy</b><br/>Threshold {activeModel?.threshold ?? 0.5}</div>
 ...
 <div className="twocol"><div className="card"><h2>Model state</h2><p>{!pred?'Waiting for a complete 30-second window':pred.abstained?'Abstained: '+pred.reason:`Stress model score: ${(pred.probabilities[1]*100).toFixed(1)}%`}</p><p className="muted">Scores are uncalibrated model outputs. No diagnosis.</p><p>Active Hash: {activeModel?.model_hash?.slice(0,8) || 'unknown'}</p><p>Threshold: {activeModel?.threshold ?? 0.5}</p><span className="badge">{session?.status||'No active session'} · {session?.speed||speed}×</span></div><div className="card"><h2>Recent decisions</h2>{events.filter(e=>e.event_type==='decision').slice(-4).reverse().map(e=><div className="row" key={e.event_id}><b>{e.payload.result}</b><span>{e.payload.reason_codes.join(', ')}</span></div>)}</div></div></>}
 ...
 {page==='History'&&mode==='user'&&<div className="card"><h2>Session history</h2><label>Inspect a saved session<select value={selected} onChange={e=>SEL(e.target.value)}><option value="">Select a session</option>{sessions.map(s=><option value={s.id} key={s.id}>{SCENARIOS[s.scenario] || s.scenario} · {s.status} · {s.created_at}</option>)}</select></label><p>History is retained in local SQLite storage.</p></div>}
 ...
 {page==='Evidence'&&mode==='research'&&<div className="card"><h2>Build evidence console</h2><p>Signals and measured model results use generated synthetic data.</p>
 <button onClick={()=>act(()=>api(`models/rollback`,{}))}>Rollback Active Model</button>
 <br/><br/>
 <button onClick={exportEvidence}>Download ZIP Evidence</button>
 </div>}
 
 {page==='Settings'&&<><div className="card"><h2>Settings</h2><label>Replay speed<select value={speed} onChange={e=>SP(+e.target.value)}><option value={1}>1x</option><option value={5}>5x</option><option value={20}>20x</option></select></label><p>Display preferences and reduced motion would go here.</p></div></>}
